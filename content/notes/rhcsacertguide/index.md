---
title: 'RHCSA Cert Guide Notes'
date: 2025-03-03
description: "My RHCSA Notes"
summary: "My RHCSA Notes"
---
<div style="display:flex; gap:1rem; align-items:flex-start;">
  <img src="rhcsaguide.png" style="width:25%; height:auto;" />
  <div>
    <h3 class="mb-1">
    </h3>
    <div class="text-2xl">
    ★★★★☆
    </div>
    <p>
    Great resource for passing the RHCSA exam. I passed first try using this and man pages alone. 
    </p>
  </div>
</div>

### My Notes
- [Advanced File Management](advancedfilemanagement/)
- [index](advancedpackagemanagement/index.md)
- [index](advancedusermanagement/index.md)
- [index](autofs/index.md)
- [index](bashshell/index.md)
- [index](basicpackagemanagement/index.md)
- [index](basicusermanagement/index.md)
- [index](bootprocessgrub2kernel/index.md)
- [index](containers/index.md)
- [index](dns/index.md)
- [index](filemanagement/index.md)
- [Installation](installation/Installation.md)
- [Interaction](Interaction.md)
- [Local File Systems and Swap](Local%20File%20Systems%20and%20Swap.md)
- [Network Devices and Connections](Network%20Devices%20and%20Connections.md)
- [Network File System (NFS)](Network%20File%20System%20(NFS).md)
- [Partitioning, MBR, and GPT](Partitioning,%20MBR,%20and%20GPT.md)
- [Process and Task Scheduling](Process%20and%20Task%20Scheduling.md)
- [Remove a filesystem from a partition](Remove%20a%20filesystem%20from%20a%20partition.md)
- [Security Enhanced Linux](Security%20Enhanced%20Linux.md)
- [Shell Scripting](Shell%20Scripting.md)
- [System Initialization, Message Logging, and System Tuning](System%20Initialization,%20Message%20Logging,%20and%20System%20Tuning.md)
- [The Linux Firewall](The%20Linux%20Firewall.md)
- [The Secure Shell Service](The%20Secure%20Shell%20Service.md)
- [Thin Provisioning](Thin%20Provisioning.md)
- [Time Synchronization](Time%20Synchronization.md)
- [Virtual Data Optimizer (VDO)](Virtual%20Data%20Optimizer%20(VDO).md)

